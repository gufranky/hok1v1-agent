...
def build_frame(agent, observation):
    ...
    for hero in hero_list:
        ...
        hero_camp = actor_state.get("camp", hero.get("camp"))
        if isinstance(hero_camp, str) and hero_camp.startswith("PLAYERCAMP_"):
            hero_camp = int(hero_camp.rsplit("_", 1)[-1]) - 1
        hero_hp = actor_state.get("hp", hero.get("hp", 0))
    ...
    frame = Frame(
        ...
        advantage=0,
        prob=prob,
        sub_action=sub_action_mask[str(action[0])] if isinstance(sub_action_mask, dict) else sub_action_mask[action[0]],
