...
class EpisodeRunner:
    def __init__(self, env, agents, logger, monitor, env_conf_manager, lineup_iterator):
        ...
        self.debug_agents = [DebugAgent(), DebugAgent()] if GameConfig.debug_agent else []
    ...
    def run_episodes(self):
        ...
        while True:
            if GameConfig.debug_agent and self.episode_cnt == GameConfig.debug_max_run_episodes:
                raise SystemExit("Finish DEBUG Agent")
                ...
                for index, (do_predict, do_sample, agent) in enumerate(
                    ...
                    if do_predict:
                        if GameConfig.debug_agent:
                            agent.observation_process(observation[str(index)])
                            actions[index] = self.debug_agents[index].act(
                                agent.parsed_state,
                                legal_actions=observation[str(index)]["legal_action"],
                                sub_action_mask=observation[str(index)]["sub_action_mask"],
                            )
                        elif not is_eval:
    ...
    def try_save_frame(self, observation):
        frame_no = observation["0"]["frame_state"].get("frame_no", observation["0"]["frame_state"].get("frameNo", 0))
        if GameConfig.debug_frames and frame_no <= GameConfig.debug_max_save_frame_no:
            path_frames = Path(__file__).parents[1] / "debug" / "frames"
            path_frames.mkdir(exist_ok=True, parents=True)
            with open(path_frames / f"{frame_no}.json", "w", encoding="utf-8") as file:
